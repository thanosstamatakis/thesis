...
const main = async () => {
  ...
  for await (post of posts.slice(post_idx)) {
    ...
    try {
      ...
      try {
        if (hasReactions) {
          await page.waitForSelector("div[aria-label=Close],div[aria-label=Κλείσιμο]");
          await page.click("div[aria-label=Close],div[aria-label=Κλείσιμο]");
        }
      } catch {
...
async function getPersonalPostReactions(page) {
  try {
    ...
    for await ([idx, height] of heights.reverse().entries()) {
      ...
      await page.click("div[aria-label=Close],div[aria-label=Κλείσιμο]", { delay: 200 });
    }
    await page.evaluate('document.querySelectorAll("div[aria-label=Close],div[aria-label=Κλείσιμο]").forEach(el=>el.click())');
...
async function scrollReactions(page) {
  try {
    await page.waitForSelector("div[aria-label=Reactions] > div:nth-child(3), div[aria-label=Αντιδράσεις] > div:nth-child(3)", { timeout: 3000 });
    let retries = 0;
    let previousScrollTop = await page.$eval(
      "div[aria-label=Reactions] > div:nth-child(3), div[aria-label=Αντιδράσεις] > div:nth-child(3)",
    ...
    while (retries <= 2) {
      ...
      page.evaluate(
        `document.querySelector('div[aria-label=Reactions] > div:nth-child(3), div[aria-label=Αντιδράσεις] > div:nth-child(3)').scrollTo(0, document.querySelector('div[aria-label=Reactions] > div:nth-child(3), div[aria-label=Αντιδράσεις] > div:nth-child(3)').scrollHeight)`
      );
      previousScrollTop = scrollTop;
    ...
    const people = await page.$$eval(
      "div[aria-label=Reactions] > div:nth-child(3) span[dir=auto]>div>a,div[aria-label=Αντιδράσεις] > div:nth-child(3) span[dir=auto]>div>a",
      (people) => people.map((person) => person?.innerText)
    );
    const reactions = await page.$$eval(
      "div[aria-label=Reactions] > div:nth-child(3) div>a div img, div[aria-label=Αντιδράσεις] > div:nth-child(3) div>a div img",
